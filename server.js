...
// ── Webhook Channex (senza auth — Channex chiama direttamente)
...
// Tutte le rotte /api/* sotto richiedono login
...
app.put('/api/quote/:id/stato', async (req, res) => {
  ...
  const { data, error } = await supabase.from('quote').update({
    stato,
...
app.post('/api/channex/create-property', async (req, res) => {
  try { res.json(await channex.client.createProperty(req.body)); }
  catch (err) { res.status(500).json({ errore: err.message }); }
});

app.post('/api/channex/create-room-type', async (req, res) => {
  try { res.json(await channex.client.createRoomType(req.body)); }
  catch (err) { res.status(500).json({ errore: err.message }); }
});

app.post('/api/channex/create-rate-plan', async (req, res) => {
  try { res.json(await channex.client.createRatePlan(req.body)); }
  catch (err) { res.status(500).json({ errore: err.message }); }
});
...
app.get('/api/channex/log', async (req, res) => {
  const { data, error } = await supabase.from('channex_log').select('*')
...
app.post('/api/channex/setup-webhook', async (req, res) => {
  ...
  if (!webhookUrl) return res.status(400).json({ errore: 'Imposta BASE_URL nel .env.' });
  try {
...
app.listen(PORT, () => {
  ...
  console.log(`Channex: ${process.env.CHANNEX_ENV === 'production' ? 'PRODUZIONE' : 'STAGING'}`);
});
