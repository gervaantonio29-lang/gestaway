...
function fetchUrl(url) {
  return new Promise((resolve, reject) => {
    ...
    client.get(url, (r) => { let data=''; r.on('data', c => data+=c); r.on('end', () => resolve(data)); }).on('error', reject);
  });
}
...
app.post('/api/sync/:id', requireAuth, async (req, res) => {
  ...
  for (const [url, fonte] of [[apt.ical_airbnb,'Airbnb'],[apt.ical_booking,'Booking']]) {
    ...
    try {
      ...
      for (const e of parseIcal(data, fonte, apt.id)) {
        if (fonte==='Airbnb') { const n=(e.ospite||'').toUpperCase(); if (n.includes('NOT AVAILABLE')||n==='CLOSED'||n==='BLOCKED') continue; }
        const { error } = await supabase.from('prenotazioni').upsert({ ...e, stato:'confermata', questura_inviata:0 }, { onConflict:'uid', ignoreDuplicates:true });
        if (!error) importati++;
      }
    } catch(e) { console.error('Errore iCal '+fonte, e.message); }
  }
...
app.post('/api/pulizia/not-available', requireAuth, async (req, res) => {
  ...
  for (const p of (prens||[])) {
    ...
    if (n.includes('NOT AVAILABLE')||n==='CLOSED'||n==='BLOCKED') { await supabase.from('prenotazioni').delete().eq('id',p.id); rimossi++; }
  }
...
function buildAlloggiatiLines(ospiti, pren) {
  ...
  return ospiti.map((o,i)=>{
    const tipo=i===0?'16':'19';
    const arrivo=fmtData(pren.data_arrivo);
    ...
    const stato=pad(o.stato_nascita_codice||'100000100',9);
    const comune=o.comune_nascita_codice?pad(o.comune_nascita_codice,9):'         ';
    const prov=o.comune_nascita_provincia?pad(o.comune_nascita_provincia,2):'  ';
    ...
    for(const pren of (prens||[])){
      ...
      try{
        ...
        if(esito==='true'||(schedineValide&&parseInt(schedineValide)>0)){
          ...
          console.log('🚔 [AUTO] Inviata pren',pren.id);
        }
...
setInterval(()=>{
  const now=new Date(), ora=now.getHours(), min=now.getMinutes();
  const k=now.toDateString()+'-'+ora;
